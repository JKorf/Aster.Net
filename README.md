...
[![Docs](https://img.shields.io/badge/Docs-Aster.Net-1b7f50?style=for-the-badge)](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Aster.Net)
...
The [Aster.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Aster.Net) is the main resource for installing, configuring, and using the library.

| Resource | Description |
|--|--|
| [Client guide](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Aster.Net) | Installation, REST and WebSocket clients, authentication, dependency injection, error handling, and advanced features |
| [Examples](https://cryptoexchange.jkorf.dev/docs/exchange-clients/examples?library=Aster.Net) | Common REST and WebSocket operations |
| [API reference](https://cryptoexchange.jkorf.dev/docs/exchange-clients/reference?library=Aster.Net) | Client interfaces, methods, and properties |
...
### NuGet 
[![NuGet version](https://img.shields.io/nuget/v/JKorf.Aster.net.svg?style=for-the-badge)](https://www.nuget.org/packages/JKorf.Aster.Net)  [![Nuget downloads](https://img.shields.io/nuget/dt/JKorf.Aster.Net.svg?style=for-the-badge)](https://www.nuget.org/packages/JKorf.Aster.Net)

	dotnet add package JKorf.Aster.Net
	
### GitHub packages
Aster.Net is available on [GitHub packages](https://github.com/JKorf/Aster.Net/pkgs/nuget/JKorf.Aster.Net). You'll need to add `https://nuget.pkg.github.com/JKorf/index.json` as a NuGet package source.

### Download release
[![GitHub Release](https://img.shields.io/github/v/release/JKorf/Aster.Net?style=for-the-badge&label=GitHub)](https://github.com/JKorf/Aster.Net/releases)
...
For more examples and explanations, continue with the [Aster.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=Aster.Net) or browse the [compilable repository examples](https://github.com/JKorf/Aster.Net/tree/main/Examples).
...
This allows the same application code to work with different exchange libraries. Each Aster API surface exposes its supported shared functionality through a `SharedClient` property. Because support differs between exchanges and API surfaces, call `Discover()` to inspect the available trading modes, environments, endpoints, and subscriptions at runtime.

### Supported shared interfaces

The V1 and V3 API surfaces currently expose the same shared interfaces. The V3 surfaces are recommended for new integrations.

| API | Type | Supported interfaces |
|--|--|--|
| `SpotApi` / `SpotV3Api` | REST | `IAssetsRestClient`, `IBalanceRestClient`, `IBookTickerRestClient`, `IFeeRestClient`, `IKlineRestClient`, `IOrderBookRestClient`, `IRecentTradeRestClient`, `ISpotOrderClientIdRestClient`, `ISpotOrderRestClient`, `ISpotSymbolRestClient`, `ISpotTickerRestClient`, `ISpotTriggerOrderRestClient`, `ITradeHistoryRestClient`, `ITransferRestClient` |
| `SpotApi` / `SpotV3Api` | WebSocket | `IBalanceSocketClient`, `IBookTickerSocketClient`, `IKlineSocketClient`, `IOrderBookSocketClient`, `ISpotOrderSocketClient`, `ITickerSocketClient`, `ITickersSocketClient`, `ITradeSocketClient` |
| `FuturesApi` / `FuturesV3Api` | REST | `IBalanceRestClient`, `IBookTickerRestClient`, `IFeeRestClient`, `IFundingRateRestClient`, `IFuturesOrderClientIdRestClient`, `IFuturesOrderRestClient`, `IFuturesSymbolRestClient`, `IFuturesTickerRestClient`, `IFuturesTpSlRestClient`, `IFuturesTriggerOrderRestClient`, `IIndexPriceKlineRestClient`, `IKlineRestClient`, `ILeverageRestClient`, `IMarkPriceKlineRestClient`, `IOrderBookRestClient`, `IPositionModeRestClient`, `IRecentTradeRestClient`, `ITradeHistoryRestClient` |
| `FuturesApi` / `FuturesV3Api` | WebSocket | `IBalanceSocketClient`, `IBookTickerSocketClient`, `IFuturesOrderSocketClient`, `IKlineSocketClient`, `IOrderBookSocketClient`, `IPositionSocketClient`, `ITickerSocketClient`, `ITickersSocketClient`, `ITradeSocketClient` |

### Discover supported functionality

```csharp
var sharedClient = new AsterRestClient().SpotV3Api.SharedClient;
...
* Version 4.1.0 - 09 Jul 2026
    * Updated CryptoExchange.Net to v12.1.0Added strategy order endpoints
    * Added Pending order status
    * Added MarginType.Cross value mapping
    * Fixed some ChaseOrder endpoint issues
...
* Version 4.0.0 - 29 Jun 2026
    ...
    * Added setter to AsterExchange.RateLimiter to allow custom rate limit settings
